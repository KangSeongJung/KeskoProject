...
public class WriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		req.getRequestDispatcher("/admin/adminwrite.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(mr == null) {
			JSFunction.alertLocation(resp, "제한용량초과 or 디렉토리 경로를 확인하세요.", "../kesboard/adminwrite.do");
			return;
		}
		...
		if(result == 1) {
			...
			resp.sendRedirect("../kesboard/tables.do");
		}
		else {
			//insert에 실패하면 작성페이지로 다시 이동한다.
			resp.sendRedirect("../kesboard/adminwrite.do");
		}
