...
public class FreeWriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(commandStr.equals("/freewrite.do")){

		req.getRequestDispatcher("/Keskomain/freeboard/freewrite.jsp").forward(req, resp);
		}
		else {
		req.getRequestDispatcher("../admin/adminfreewrite.jsp").forward(req, resp);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(mr == null) {
			JSFunction.alertLocation(resp, "제한용량초과 or 디렉토리 경로를 확인하세요.", "../freeboard/freewrite.do");
			return;
		}
		...
		if(result == 1) {
			...
			if(commandStr.equals("/freewrite.do")){
				resp.sendRedirect("../freeboard/freelist.do");
			}
			else {
				resp.sendRedirect("../freeboard/adminfreelist.do");		}
		}
		else {
			//insert에 실패하면 작성페이지로 다시 이동한다.
			resp.sendRedirect("../freeboard/freewrite.do");
		}
